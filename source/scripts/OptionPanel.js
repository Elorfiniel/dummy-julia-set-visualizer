...
function galleryJulia(target) {
  ...
  var galleryList = [];
  eval("galleryList = " + galleryCollect.value + ";");
...
function miniMap() {
  ...
  context.strokeRect(
    (192 - pDel) / 2 + 48 * xTrans,
    (192 - pDel) / 2 + 48 * yTrans,
    pDel,
    pDel
  );
  context.fillRect(
...
waterMark.addEventListener("focus", function () {
  keyBoard = false;
});
waterMark.addEventListener("blur", function () {
  keyBoard = true;
});
...
wmSwitch.addEventListener("click", function () {
  if (!Boolean(wmSwitch.checked)) {
    ...
    setTimeout(function () {
      waterMark.placeholder = "Watermark OFF";
      waterMark.value = "";
    }, 200);
    waterMark.title = "Turn on the switch to add watermark";
    waterMark.setAttribute(
      "class",
      waterMark.getAttribute("class").replace("textBox", "textBox_off")
    );
  } else {
    waterMark.disabled = false;
    waterMark.placeholder = "Designed by Elorfiniel";
    waterMark.title = "Alphanumerical Characters or Spaces";
    waterMark.setAttribute(
      "class",
      waterMark.getAttribute("class").replace("textBox_off", "textBox")
    );
  }
