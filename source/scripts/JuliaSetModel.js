function renderJulia(
  ...
  var depth = 100;
  var xScl = (2 * cScaleX) / cWidth;
  var yScl = (2 * cScaleY) / cHeight;
  var theshold = 4;
  var cJulia = [rValue, iValue];
  // Preparation for sketching, including scale and iteration depth
  var juliaOuterRing = 2;
  var juliaGradOuter = 16;
  var juliaGradInner = 92;
  var juliaRim = 4;
  var juliaHCGrad = 5.6;
  var juliaLCGrad = 0.92;
  ...
  function cMulti(cNum1, cNum2) {
    return [
      cNum1[0] * cNum2[0] - cNum1[1] * cNum2[1],
      cNum1[0] * cNum2[1] + cNum1[1] * cNum2[0],
    ];
  ...
  function mapLight(col) {
    if (col <= juliaOuterRing) {
      var gradLight =
        lightValue > 50 ? (lightValue - 50) / 2.4 : (lightValue - 50) / 4;
      return juliaRim + gradLight <= 1 ? 1 : juliaRim + gradLight;
    } else if (col <= juliaGradOuter) {
      var gradLight =
        lightValue > 50 ? (lightValue - 50) / 2.4 : (lightValue - 50) / 3.6;
      ...
      var gradLight =
        lightValue > 50 ? (lightValue - 50) / 2.4 : (lightValue - 50) / 3.2;
      if (lightValue < 10) gradLight *= 1.8;
      else if (lightValue < 20) gradLight *= 1.4;
      var temp =
        (col - juliaGradOuter) * juliaLCGrad +
        14 * juliaHCGrad +
        juliaRim +
        gradLight;
  ...
  function plotJulia() {
    ...
    for (var iteS = 0; iteS < cWidth; iteS += cDense) {
      ...
      for (var iteJ = 0; iteJ < cHeight; iteJ += cDense) {
        var point = [
          iteS * xScl - cScaleX + xDel,
          iteJ * yScl - cScaleY + yDel,
        ];
        ...
        if (bound != preBound) {
          if (bound == depth) {
            ...
            context.fillStyle = `hsl(${hueValue},${saturValue}%,${mapLight(
              bound
            )}%)`;
            preBound = bound;
          }
        } // Set fillStyle with HEX colors, no need to set strokeStyle
        context.fillRect(iteS, iteJ, cDense, cDense);
      }
    }
  } // Plot Julia Set
