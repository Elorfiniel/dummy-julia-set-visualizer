...
function randomize(
  ...
  if (rPartTag == true)
    rPart.value = (Math.random() * 2 * Math.sign(Math.random() - 0.5)).toFixed(
      2
    );
  if (iPartTag == true)
    iPart.value = (Math.random() * 2 * Math.sign(Math.random() - 0.5)).toFixed(
      2
    );
...
function animationGen(requiree = "", method = "") {
  ...
  var lowerBound = Number(target.min);
  var upperBound = Number(target.max);
  var pace = Number(target.step);
  var current = Number(target.value);
  if (method == "linear") {
...
function modifyPanelClass(target = "", prohibit = "") {
  ...
  if (prohibit == true) {
    mod1.setAttribute(
      "class",
      mod1.getAttribute("class").replace("switch_botton", "switch_botton_off")
    );
    mod2.setAttribute(
      "class",
      mod2.getAttribute("class").replace("switch_botton", "switch_botton_off")
    );
    if (Boolean(mod3.checked)) mod3.checked = false;
    mod3.disabled = true;
    mod4.setAttribute("class", "animate_wrapper_off");
    mod5.disabled = true;
    mod5.setAttribute("class", "rangeSlider_off");
    eval(target.toLowerCase() + "Animator_off = true;");
  } else {
    mod1.setAttribute(
      "class",
      mod1.getAttribute("class").replace("switch_botton_off", "switch_botton")
    );
    mod2.setAttribute(
      "class",
      mod2.getAttribute("class").replace("switch_botton_off", "switch_botton")
    );
    mod3.checked = false;
    mod3.disabled = false;
    mod4.setAttribute("class", "animate_wrapper");
    mod5.disabled = false;
    mod5.setAttribute("class", "rangeSlider");
  }
